...
def _calc_varday__dom_ressurectionis(year):
    """ Dominica Ressurectionis - Easter Sunday """
    return easter(year)

def _calc_varday__dom_sanctae_familiae(year):
...
def _calc_varday__dom_septuagesima(year):
    ...
    """
    return _calc_varday__dom_ressurectionis(year) - timedelta(days=63)

def _calc_varday__dom_adventus(year):
    """ Dominica I Adventus
...
def _calc_varday__dom_post_pentecost_24(year):
    ...
    * directly after week of dom_post_epiphania_6 (moved from post
    ...
    """
    return _calc_varday__dom_adventus(year) - timedelta(days=7)

def _calc_varday__sab_before_dom_post_pentecost_24(year):
    ...
    """
    return _calc_varday__dom_post_pentecost_24(year) - timedelta(days=1)

def _calc_varday__quattour_septembris(year):
...
def _calc_varday__sanctissimi_nominis_jesu(year):
...
def _calc_varday__jesu_christi_regis(year):
    """ Jesu Christi Regis
...
def _calc_varday__dom_octavam_nativitatis(year):
...
def _calc_fixday__11_02_omnium_fidelium_defunctorum(year):
...
def _calc_fixday__02_24_matthiae_apostoli(year):
    """ Matthiae Apostoli
...
class MissalFactory(object):
    ...
    def create(cls, year):
        ...
        cls._fill_in_variable_days(year)
        cls._fill_in_fixed_days()
        cls._fill_in_semi_fixed_days(year)
        cls._resolve_concurrency()
        return cls.missal

    @classmethod
    def _fill_in_variable_days(cls, year):
        """
        Days depending on variable date, such as Easter or Advent
        """
        # main blocks
        cls._insert_block(
            _calc_varday__dom_sanctae_familiae(year),
            blocks.VARDAYS__POST_EPIPHANIA)
        cls._insert_block(
            _calc_varday__dom_septuagesima(year),
            blocks.VARDAYS__RESSURECTIONIS)
        cls._insert_block(
            _calc_varday__sab_before_dom_post_pentecost_24(year),
            blocks.VARDAYS__POST_EPIPHANIA,
            reverse=True,
            overwrite=False)
        cls._insert_block(
            _calc_varday__dom_post_pentecost_24(year),
            blocks.VARDAYS__HEBD_POST_PENTECOST_24)
        cls._insert_block(
            _calc_varday__dom_adventus(year),
            blocks.VARDAYS__ADVENT,
            stop_date=date(year, 12, 23))
        # additional blocks
        cls._insert_block(
            _calc_varday__sanctissimi_nominis_jesu(year),
            blocks.VARDAYS__SANCTISSIMI_NOMINIS_JESU
        )
        cls._insert_block(
            _calc_varday__quattour_septembris(year),
            blocks.VARDAYS__QUATTOUR_SEPTEMBRIS)
        cls._insert_block(
            _calc_varday__jesu_christi_regis(year),
            blocks.VARDAYS__JESU_CHRISTI_REGIS
        )
        if _calc_varday__dom_octavam_nativitatis(year):
            cls._insert_block(
                _calc_varday__dom_octavam_nativitatis(year),
                blocks.VARDAYS__DOM_OCTAVAM_NATIVITATIS
            )

    @classmethod
    def _fill_in_fixed_days(cls):
        """
    ...
    @classmethod
    def _fill_in_semi_fixed_days(cls, year):
        """
        Days normally ascribed to specific date, but in
        certain conditions moved to other dates
        """
        day = _calc_fixday__11_02_omnium_fidelium_defunctorum(year)
        cls.missal[day].append(
            LiturgicalDay(FIX_11_02_OMNIUM_FIDELIUM_DEFUNCTORUM, day))
        cls.missal[day].sort(reverse=True)

        day = _calc_fixday__02_24_matthiae_apostoli(year)
        cls.missal[day].append(LiturgicalDay(FIX_02_24_MATTHIAE_APOSTOLI, day))
        cls.missal[day].sort(reverse=True)

        day = _calc_fixday__02_27(year)
        cls.missal[day].append(LiturgicalDay(FIX_02_27_1, day))
        cls.missal[day].sort(reverse=True)
    ...
    def _insert_block(cls, start_date, block, stop_date=None,
        ...
        ...
          ...
          datetime.date(2008, 1, 14): [<var:f2_post_epiphania_1:4>,
                                       <fix:01-14_1:3>],
          datetime.date(2008, 1, 15): [<var:f3_post_epiphania_1:4',
                                       <fix:01-15_1:3>],
        ...
        }
