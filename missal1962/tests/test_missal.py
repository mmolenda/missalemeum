...
class TestMissal(unittest.TestCase):
    ...
    def test_temporadays(self):
        ...
        for year, dates in sorted(expected.iteritems()):
            ...
            self.assertEqual(self._to_date_obj(dates[10]), missal.get_day_by_id(SANCTI_10_DU)[0])
    ...
    def test_concurrency_12_08_conceptione_immaculata_bmv(self):
        self.assertEqual([SANCTI_12_08_CONCEPTIONE_IMMACULATA_BMV, TEMPORA_ADV2_0],
                         [i.id for i in MissalFactory.create(1907)[date(1907, 12, 8)]])
        self.assertEqual([SANCTI_12_08_CONCEPTIONE_IMMACULATA_BMV, TEMPORA_ADV2_0],
                         [i.id for i in MissalFactory.create(1912)[date(1912, 12, 8)]])
        self.assertEqual([SANCTI_12_08_CONCEPTIONE_IMMACULATA_BMV, TEMPORA_ADV2_1],
    ...
    def test_liturgical_day_model_temporaiable_rank_advent(self):
        # 2002
        self.assertEqual(LiturgicalDay(TEMPORA_ADV3_1, date(2015, 12, 16)).rank, 3)
        self.assertEqual(LiturgicalDay(TEMPORA_ADV3_2, date(2015, 12, 17)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_ADV3_3, date(2015, 12, 18)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_ADV3_4, date(2015, 12, 19)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_ADV3_5, date(2015, 12, 20)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_ADV3_6, date(2015, 12, 21)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_ADV4_0, date(2015, 12, 22)).rank, 1)
        self.assertEqual(LiturgicalDay(TEMPORA_ADV4_1, date(2015, 12, 23)).rank, 2)
        # 2015
