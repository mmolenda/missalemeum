...
class TestMissal(unittest.TestCase):
    ...
    def test_temporadays(self):
        ...
        for year, dates in sorted(expected.iteritems()):
            ...
            self.assertEqual(self._to_date_obj(dates[0]), missal.get_day_by_id(TEMPORA_DOM_SEPTUAGESIMA)[0])
            self.assertEqual(self._to_date_obj(dates[1]), missal.get_day_by_id(TEMPORA_F4_CINERUM)[0])
            self.assertEqual(self._to_date_obj(dates[2]), missal.get_day_by_id(TEMPORA_DOM_RESURRECTIONIS)[0])
            self.assertEqual(self._to_date_obj(dates[3]), missal.get_day_by_id(TEMPORA_ASCENSIONE_DOMINI)[0])
            ...
            self.assertEqual(self._to_date_obj(dates[6]), missal.get_day_by_id(TEMPORA_DOM_ADVENTUS_1)[0])
            # dom_post_epiphania_4 might not exist in given year, then None is returned
            actual = missal.get_day_by_id(TEMPORA_DOM_POST_EPIPHANIA_4)[0] if \
                missal.get_day_by_id(TEMPORA_DOM_POST_EPIPHANIA_4) else None
            self.assertEqual(self._to_date_obj(dates[7]), actual)
            self.assertEqual(self._to_date_obj(dates[8]), missal.get_day_by_id(TEMPORA_SAB_QUATTUOR_SEPTEMBRIS)[0])
            ...
            actual = missal.get_day_by_id(TEMPORA_DOM_OCTAVAM_NATIVITATIS)[0] if \
                missal.get_day_by_id(TEMPORA_DOM_OCTAVAM_NATIVITATIS) else None
    ...
    def test_concurrency_12_08_conceptione_immaculata_bmv(self):
        self.assertEqual([SANCTI_12_08_CONCEPTIONE_IMMACULATA_BMV, TEMPORA_DOM_ADVENTUS_2],
                         [i.id for i in MissalFactory.create(1907)[date(1907, 12, 8)]])
        self.assertEqual([SANCTI_12_08_CONCEPTIONE_IMMACULATA_BMV, TEMPORA_DOM_ADVENTUS_2],
                         [i.id for i in MissalFactory.create(1912)[date(1912, 12, 8)]])
        self.assertEqual([SANCTI_12_08_CONCEPTIONE_IMMACULATA_BMV, TEMPORA_F2_ADVENTUS_2],
    ...
    def test_liturgical_day_model_simple_case(self):
        self.assertEqual(LiturgicalDay(TEMPORA_F4_POST_EPIPHANIA_2, date(2002, 1, 23)).rank, 4)
        self.assertEqual(LiturgicalDay(TEMPORA_F4_POST_EPIPHANIA_2, date(2002, 1, 23)).weekday, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_DOM_SEPTUAGESIMA, date(2002, 1, 27)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_DOM_SEPTUAGESIMA, date(2002, 1, 27)).weekday, 6)
        self.assertEqual(LiturgicalDay(TEMPORA_DOM_SEPTUAGESIMA, date(2002, 1, 28)).weekday, 6)

    def test_liturgical_day_model_temporaiable_rank_advent(self):
        # 2002
        self.assertEqual(LiturgicalDay(TEMPORA_F2_ADVENTUS_3, date(2015, 12, 16)).rank, 3)
        self.assertEqual(LiturgicalDay(TEMPORA_F3_ADVENTUS_3, date(2015, 12, 17)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_F4_QUATTUOR_ADVENTUS, date(2015, 12, 18)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_F5_ADVENTUS_3, date(2015, 12, 19)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_F6_QUATTUOR_ADVENTUS, date(2015, 12, 20)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_SAB_QUATTUOR_ADVENTUS, date(2015, 12, 21)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_DOM_ADVENTUS_4, date(2015, 12, 22)).rank, 1)
        self.assertEqual(LiturgicalDay(TEMPORA_F2_ADVENTUS_4, date(2015, 12, 23)).rank, 2)
        # 2015
        self.assertEqual(LiturgicalDay(TEMPORA_F3_ADVENTUS_3, date(2015, 12, 15)).rank, 3)
        self.assertEqual(LiturgicalDay(TEMPORA_F4_QUATTUOR_ADVENTUS, date(2015, 12, 16)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_F5_ADVENTUS_3, date(2015, 12, 17)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_F6_QUATTUOR_ADVENTUS, date(2015, 12, 18)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_SAB_QUATTUOR_ADVENTUS, date(2015, 12, 19)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_DOM_ADVENTUS_4, date(2015, 12, 20)).rank, 1)
        self.assertEqual(LiturgicalDay(TEMPORA_F2_ADVENTUS_4, date(2015, 12, 21)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_F3_ADVENTUS_4, date(2015, 12, 22)).rank, 2)
        self.assertEqual(LiturgicalDay(TEMPORA_F4_ADVENTUS_4, date(2015, 12, 23)).rank, 2)
    ...
    def test_liturgical_day_compare(self):
        ...
        rank_3_1 = LiturgicalDay(TEMPORA_F6_PASSIONIS, date(2015, 3, 27))
        rank_3_2 = LiturgicalDay(SANCTI_03_28_1, date(2015, 3, 28))
        rank_4_1 = LiturgicalDay(TEMPORA_F2_POST_PENTECOST_1, date(2015, 6, 1))
