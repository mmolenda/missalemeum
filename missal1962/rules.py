...
def rule_immaculate_coneption(date_, tempora, lit_days):
...
def rule_nativity(date_, tempora, lit_days):
...
def rule_st_matthias(date_, tempora, lit_days):
...
def rule_feb27(date_, tempora, lit_days):
...
def rule_all_souls(date_, tempora, lit_days):
...
def rule_lord_feast1(date_, tempora, lit_days):
...
def rule_lord_feast2(date_, tempora, lit_days):
...
def first_class_feast_no_commemoration(date_, tempora, lit_days):
    if match(lit_days, PATTERN_CLASS_1):
        return [match(lit_days, PATTERN_CLASS_1)], [], []


def rule_2nd_class_sunday(date_, tempora, lit_days):
...
def rule_1st_class_feria(date_, tempora, lit_days):
...
def rule_2nd_class_feast_takes_over_advent_feria_and_ember_days(date_, tempora, lit_days):
...
def rule_bmv_office_on_saturday(date_, tempora, lit_days):
    pass


def rule_precedence(date_, tempora, lit_days):
    if len(lit_days) == 0:
        return [], [], []
    elif len(lit_days) == 1:
        return lit_days, [], []
